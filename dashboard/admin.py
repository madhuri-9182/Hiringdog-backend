...
class InternalInterviewerAdmin(admin.ModelAdmin):
    ...
    list_filter = ("interviewer_level", InterviewerDetailedExperienceFilter)
    list_per_page = 20
...
class JobAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        "job_role__name",
        "hiring_manager__organization__name",
    )
    ordering = ("-created_at",)
...
class JobInterviewRoundAdmin(admin.ModelAdmin):
    ...
    list_per_page = 20
    search_fields = ("job__job_role__name", "name")
    readonly_fields = ["created_at", "updated_at"]
    list_filter = (
        "job__job_role__name",
        "job__hiring_manager__organization__name",
    )

    def get_queryset(self, request):
        return self.model.object_all.select_related(
            "job__hiring_manager__organization", "job__job_role"
        )
...
class CandidateAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return self.model.object_all.select_related("organization", "specialization")
...
@admin.register(CreditPackagePricing)
class CreditPackagePricingAdmin(admin.ModelAdmin):
    list_display = (
        "package",
        "country",
        "currency",
    ...
    search_fields = ("package__name", "country", "currency")
...
@admin.register(ClientCreditWallet)
class ClientCreditWalletAdmin(admin.ModelAdmin):
    list_display = (
        "id",
        "client",
        "total_credits",
        ...
        "total_spend",
        "credit_expired_at",
        "get_pricing_plan_name",
        "archived",
    )
    search_fields = ("client__name", "pricing_plan__package__name")
    list_filter = ("client__name", "pricing_plan__package__name")
    list_per_page = 20
    ...
    def get_pricing_plan_name(self, obj):
        return obj.pricing_plan.package.name

    get_pricing_plan_name.short_description = "Pricing Plan"


@admin.register(ClientCreditTransaction)
class ClientCreditTransactionAdmin(admin.ModelAdmin):
    list_display = (
        "id",
        "get_client_name",
        "amount",
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related("wallet__client")
...
class AgreeementAdmin(admin.ModelAdmin):
    list_display = (
        "id",
        "years_of_experience",
        "rate",
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("organization__internal_client")
        )

    def country(self, obj):
        return constants.COUNTRY_DETAILS.get(
            obj.organization.internal_client.code, {"name": "India"}
        )["name"]
