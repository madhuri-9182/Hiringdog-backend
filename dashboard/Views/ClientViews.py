...
from services.interview_scheduling import (
    InterviewAvailablitySchedulingService,
)
...
from ..tasks import send_schedule_engagement_email, send_mail, send_scheduling_link_to_candidate
...
class ClientUserView(APIView, LimitOffsetPagination):
    ...
    def get(self, request, **kwargs):
        ...
        client_users = (
            ClientUser.objects.filter(organization=organization)
            .select_related("user")
            .prefetch_related("jobs__job_role")
        )
        ...
        client_users = client_users.prefetch_related("jobs__job_role")
...
@extend_schema(tags=["Job Role"])
class JobRoleView(APIView, LimitOffsetPagination):
    serializer_class = JobRoleSerializer
    permission_classes = [IsAuthenticated, IsClientAdmin | IsClientOwner | IsClientUser]

    def get_organization(self):
        return self.request.user.clientuser.organization

    def get_object(self):
        try:
            return JobRole.objects.get(
                organization=self.get_organization(), pk=self.kwargs["job_role_id"]
            )
    ...
    def error_response(
        ...
        response_data = {"status": "failed", "message": message}
        if errors:
            response_data["errors"] = errors
        return Response(response_data, status=status_code)

    def get(self, request, *args, **kwargs):
        job_role_id = kwargs.get("job_role_id")
        ...
        if job_role_id:
            ...
            return self.success_response(
                "Job Role retrieved successfully.", serializer.data
            )

        # Get paginated list
        queryset = JobRole.objects.filter(organization=self.get_organization())
        if q:
        ...
        return self.success_response("Retrieved Successfully", paginated_data.data)
    ...
    def post(self, request, *args, **kwargs):
        ...
        return self.error_response("Invalid data.", serializer.errors)

    def patch(self, request, *args, **kwargs):
        if "job_role_id" not in kwargs:
        ...
        if serializer.is_valid():
            serializer.save()
            return self.success_response(
                "Job Role updated successfully.", serializer.data
            )
...
class StreamView(APIView, LimitOffsetPagination):
...
class JobView(APIView, LimitOffsetPagination):
    ...
    def get(self, request, **kwargs):
        ...
        job_role_ids = request.query_params.get("job_role_ids")
        ...
        try:
            job_role_ids = (
                [int(i) for i in job_role_ids.split(",")] if job_role_ids else []
            )
        except ValueError:
            return Response(
                {
        ...
        jobs = (
            Job.objects.filter(
                Q(hiring_manager__organization=org_id)
                | Q(hiring_manager__organization=request.user.clientuser.organization)
            )
            .select_related(
                "department", "job_role", "hiring_manager", "specialization"
            )
        ...
        if job_role_ids:
            jobs = jobs.filter(job_role__in=job_role_ids)
...
class CandidateView(APIView, LimitOffsetPagination):
    ...
    def get(self, request, **kwargs):
        ...
        # Early validation with combined error handling
        validation_errors = self._validate_query_params(
            status_, job_role_id, department_uid, request
        )
        if validation_errors:
            return validation_errors

        # Get base queryset with all necessary optimizations
        candidates = self._get_optimized_queryset(request)
        ...
        # Handle single candidate request
        if candidate_id:
            return self._handle_single_candidate(candidates, candidate_id)

        # Get statistics with single aggregated query
        stats = self._get_candidate_statistics(
            candidates.filter(organization=request.user.clientuser.organization)
        )
    ...
    def _validate_query_params(self, status_, job_role_id, department_uid, request):
        """Validate all query parameters in one method"""
        ...
        # Domain designation validation
        if job_role_id:
            if not job_role_id.isdigit():
                return Response(
                    {
                        "status": "failed",
                        "message": "Invalid job_id format in query_params. It should be a integer.",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def _get_optimized_queryset(self, request):
        """Get the base queryset with all necessary optimizations"""
        candidates = (
            Candidate.objects.filter(organization=request.user.clientuser.organization)
            .select_related(
                "designation__job_role",
    ...
    def _apply_filters(
        ...
        if job_role_id:
            filters &= Q(designation__job_role_id=job_role_id)
        ...
        if status_:
            if status_ == "SCH":
                ...
                filters &= Q(Q(status=status_) | Q(final_selection_status=status_))

        if specialization_id:
            filters &= Q(specialization_id=specialization_id)

        if department_uid:
            filters &= Q(designation__department_id=department_uid)
        ...
        return candidates.filter(filters) if filters else candidates

    def _get_candidate_statistics(self, base_queryset):
        """Get all statistics in a single aggregated query"""
        stats = base_queryset.aggregate(
            total_candidates=Count("id"),
            scheduled=Count("id", filter=Q(status__in=["SCH", "CSCH"])),
            inprocess=Count("id", filter=Q(status="NSCH")),
            recommended=Count("id", filter=Q(status__in=["REC", "HREC"])),
            rejected=Count("id", filter=Q(status__in=["SNREC", "NREC"])),
        )
    ...
    def _handle_single_candidate(self, candidates, candidate_id):
        """Handle single candidate request efficiently"""
        try:
    ...
    def post(self, request, **kwargs):
        is_send_link = get_boolean(request.query_params, "is_send_link")
        serializer = self.serializer_class(
            data=request.data,
        ...
        if serializer.is_valid():
            ...
            # send the scheduling link to candidate if possible
            if is_send_link:
                send_scheduling_link_to_candidate.delay(candidate.id)
...
@extend_schema(tags=["Client"])
class RecruiterInterviewerAvailabilityView(APIView):
    ...
    def __init__(self, **kwargs):
        ...
        self.scheduling_service = InterviewAvailablitySchedulingService()

    def get(self, request):
        """Get available interview slots for a candidate"""

        # Validate query parameters
        if validation_result := self.scheduling_service.validate_query_params(
            request.query_params, is_candidate_view=False
        ):
            return validation_result

        # Parse and validate date/time
        date_time_result = self.scheduling_service.parse_and_validate_datetime(
        ...
        # Validate experience
        experience_result = self.scheduling_service.validate_experience(
            request.query_params.get("experience_year"),
            request.query_params.get("experience_month"),
        )
        if isinstance(experience_result, Response):
            return experience_result
        experience_year, experience_month = experience_result

        # Validate credits
        required_credits = self.scheduling_service.calculate_required_credits(
            experience_year, experience_month
        )
        credit_validation = self.scheduling_service.validate_sufficient_credit(
            required_credits, request.user.clientuser.organization
        )
        if isinstance(credit_validation, Response):
            return credit_validation

        # Validate specialization
        specialization_id = request.query_params.get("specialization_id")
        validated_reponse = self.scheduling_service.validate_specialization_id(
            specialization_id
        )
        if validated_reponse:
            return validated_reponse

        # Get and validate job
        job_result = self.scheduling_service.get_job(
            request.query_params.get("designation_id"),
            request.user.clientuser.organization,
        )
        ...
        if candidate_id := request.query_params.get("candidate_id"):
            candidate_result = self.scheduling_service.get_candidate(
                candidate_id, request.user.clientuser.organization
            )
        ...
        if not skills:
            return self.scheduling_service._error_response(
                "No mandatory skills found for this job."
            )

        # Get availability
        availability = self.scheduling_service.get_interviewer_availability(
            ...
            client_brand_name=request.user.clientuser.organization.internal_client.brand_name,
...
class ClientDashboardView(APIView):
    ...
    def get(self, request):
        ...
        ).annotate(name=F("job_role__name"), role_id=F("job_role_id"))
        ...
        # Job role aggregates
        job_role_aggregates = all_jobs.values("role_id", "name").annotate(
            count=Count(
                "id",
...
class ClientCreditPackagePricingView(APIView):
    serializer_class = ClientCreditPackagePricingSerializer
    permission_classes = [IsAuthenticated, IsClientOwner]
    ...
    def get(self, request):
        ...
        if not country_code:
            return Response(
                {
                    "status": "failed",
                    "message": "Country code is required in query_param.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        credit_package_pricings = CreditPackagePricing.objects.filter(
            country=country_code, is_active=True
        ).select_related("package")
        if not credit_package_pricings.exists():
            return Response(
                {
                    "status": "failed",
                    "message": "No credit package pricing found for the specified country.",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        serializer = self.serializer_class(credit_package_pricings, many=True)
        return Response(
            {
                "status": "success",
...
class ClientCreditWalletView(APIView, LimitOffsetPagination):
    permission_classes = [IsAuthenticated, IsClientOwner]
    serializer_class = ClientCreditWalletSerializer

    def get(self, request):
        wallet = (
            ClientCreditWallet.objects.filter(
                client=request.user.clientuser.organization
            )
            .select_related("client", "pricing_plan__package")
            .first()
        )
        if not wallet:
            return Response(
                {"status": "failed", "message": "Client wallet not found."},
                status=status.HTTP_404_NOT_FOUND,
            )

        serializer = self.serializer_class(wallet)
        response_data = serializer.data
        recent_transactions = ClientCreditTransaction.objects.filter(
            wallet=wallet,
            created_at__year=timezone.now().year,
            created_at__month=timezone.now().month,
        ).order_by("-created_at")[:10]
        paginated_transaction_queryset = self.paginate_queryset(
            recent_transactions, request
        )
        recent_transaction = ClientCreditTransactionSerializer(
            paginated_transaction_queryset, many=True
        )
        paginated_transaction_response = self.get_paginated_response(
            recent_transaction.data
        )
        response_data.update(**paginated_transaction_response.data)

        return Response(
            {
                "status": "success",
                "message": "Client credit wallet details retrieved successfully.",
                **response_data,
            },
            status=status.HTTP_200_OK,
        )
...
class ClientCreditTransactionView(APIView, LimitOffsetPagination):
    permission_classes = [IsAuthenticated, IsClientOwner]
    serializer_class = ClientCreditTransactionSerializer

    def get(self, request):
        transactions = ClientCreditTransaction.objects.filter(
            wallet__client=request.user.clientuser.organization
        ).order_by("-created_at")
        paginated_transactions = self.paginate_queryset(transactions, request)
        serializer = self.serializer_class(paginated_transactions, many=True)
        response_data = self.get_paginated_response(serializer.data)
        return Response(
            {
                "status": "success",
                "message": "Client credit transactions retrieved successfully.",
                **response_data.data,
            },
            status=status.HTTP_200_OK,
        )
