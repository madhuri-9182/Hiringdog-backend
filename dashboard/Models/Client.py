...
class JobRole(CreateUpdateDateTimeAndArchivedField):
    objects = SoftDelete()
    object_all = models.Manager()

    name = models.CharField(max_length=155, blank=True)
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="jobrole"
    )
...
class Job(CreateUpdateDateTimeAndArchivedField):
    ...
    job_role = models.ForeignKey(
        JobRole, on_delete=models.CASCADE, related_name="jobs", blank=True, null=True
    )
    specialization = models.ForeignKey(
        Stream, on_delete=models.SET_NULL, related_name="jobs", null=True, blank=True
    )
...
class Candidate(CreateUpdateDateTimeAndArchivedField):
    ...
    specialization = models.ForeignKey(
        Stream, on_delete=models.SET_NULL, related_name="candidates", null=True, blank=True
    )
    status = models.CharField(
        max_length=15,
        choices=STATUS_CHOICES,
    ...
    @staticmethod
    def required_credits(year, month):
