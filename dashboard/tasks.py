...
from .models import EngagementOperation, Interview, InterviewFeedback, Candidate
    ...
    **kwargs,
    ...
    try:
        ...
        email_message = EmailMultiAlternatives(
            ...
            [to],
            reply_to=reply_to if isinstance(reply_to, list) else [reply_to],
            bcc=[bcc],
        )
        ...
        contexts = [
            ...
            },
            {
                ...
                "position": get_display_name(
                    interview.candidate.designation.job_role.name,
...
def generate_interview_feedback_pdf(self, interview_uid):
    ...
    if response.status_code == 200:
        ...
        designation = get_display_name(
            candidate.designation.job_role.name, constants.ROLE_CHOICES
        )
...
@shared_task(bind=True, retry_backoff=5, max_retries=3)
def send_scheduling_link_to_candidate(self, candidate_id):
    from services.interview_scheduling import CandidateInterviewSchedulingService

    candidate = (
        Candidate.objects.select_related(
            "designation__job_role", "added_by__user", "organization", "next_round"
        )
    ...
    if not candidate:
        self.retry(exc=Exception("Candidate is not saved yet"))

    scheduling_token = CandidateInterviewSchedulingService.generate_scheduling_token(
        candidate.id
    )

    context = {
        "to": candidate.email,
        "subject": "You're Shortlisted! Schedule Your Interview Before It's Too Late",
        "template": "candidate_interview_scheduling_link.html",
        "reply_to": [CONTACT_EMAIL, candidate.added_by.user.email],
        "candidate_name": candidate.name,
        "job_title": get_display_name(
            candidate.designation.job_role.name, constants.ROLE_CHOICES
        ),
        "site_domain": settings.SITE_DOMAIN,
        "scheduling_link": f"/scheduling/{scheduling_token}/",
        "interview_duration": f"{candidate.next_round.duration_minutes} minutes",
        "interview_mode": "Virtual",
    }

    send_mail.delay(**context)

    return f"Sent scheduling link to {candidate.name} for {candidate.organization.name}"
