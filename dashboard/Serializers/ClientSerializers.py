...
from .InternalSerializers import StreamSerializer
...
from hiringdogbackend.utils import (
    ...
    validate_attachment,
    validate_json,
    set_choice_field_error_messages,
)
from ..tasks import (
    send_mail,
    send_schedule_engagement_email,
)
...
class JobSpecificDetailsSerializer(serializers.ModelSerializer):
    name = serializers.CharField(source="job_role.name", read_only=True)
...
class ClientUserSerializer(serializers.ModelSerializer):
    ...
    phone = PhoneNumberField(write_only=True, required=False, allow_null=True)
    ...
    def validate(self, data):
        errors = validate_incoming_data(
            ...
            allowed_keys=["jobs_assigned", "phone"],
...
class JobSerializer(serializers.ModelSerializer):
    ...
    role = JobRoleSerializer(source="job_role", read_only=True)
    specialization = StreamSerializer(read_only=True)
    ...
    def validate(self, data):
        ...
        if department_uid:
            try:
                data["department"] = Department.objects.get(
                    organization=org, pk=department_uid
                )
        ...
        if role_id:
            try:
                data["job_role"] = JobRole.objects.get(organization=org, pk=role_id)
            except JobRole.DoesNotExist:
                errors.setdefault("role_id", []).append("Invalid role_id")

        if specialization_id:
            try:
                data["specialization"] = Stream.objects.get(pk=specialization_id)
            except Stream.DoesNotExist:
                errors.setdefault("specialization_id", []).append(
                    "Invalid specialization_id"
                )
...
class CandidateSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        choice_fields = {
            "gender": Candidate.GENDER_CHOICES,
            "source": Candidate.SOURCE_CHOICES,
            "status": Candidate.STATUS_CHOICES,
            "final_selection_status": Candidate.FINAL_SELECTION_STATUS_CHOICES,
            "reason_for_dropping": Candidate.REASON_FOR_DROPPING_CHOICES,
            "interview_type": Candidate.INTERVIEW_TYPE_CHOICES,
        }
        set_choice_field_error_messages(self, choice_fields)

    def validate(self, data):
        """Main validation keeping original error handling structure"""
        ...
        # Use your original validation function
        errors = validate_incoming_data(
            self.initial_data,
            required_keys,
            allowed_keys,
        ...
        if data.get("job_id"):
            job = (
                Job.objects.select_related("hiring_manager")
                .filter(
                    pk=data.get("job_id"),
        ...
        if specialization_id := data.get("specialization_id"):
            try:
                data["specialization"] = Stream.objects.get(pk=specialization_id)
            except Stream.DoesNotExist:
                errors.setdefault("specialization_id", []).append(
...
class FinanceCandidateSerializer(serializers.ModelSerializer):
    role = serializers.CharField(source="designation.job_role.name")
...
class ClientCreditPackageSerializer(serializers.ModelSerializer):
    class Meta:
        model = CreditPackage
        fields = ("name", "description")


class ClientCreditPackagePricingSerializer(serializers.ModelSerializer):
    package = ClientCreditPackageSerializer(read_only=True)
...
class ClientCreditTransactionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("description", "credits", "transaction_type", "status")


class ClientCreditWalletSerializer(serializers.ModelSerializer):
    pricing_plan = serializers.CharField(
        source="pricing_plan.package.name", read_only=True
    )
    credit_expired_at = serializers.DateTimeField(format="%d/%m/%Y")

    class Meta:
        model = ClientCreditWallet
        fields = (
            "pricing_plan",
            "total_credits",
            "total_added",
