...
from hiringdogbackend.utils import validate_incoming_data, validate_attachment, get_display_name
...
class InterviewerRequestSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Validate candidate
        candidate = self._validate_candidate(data, request, errors)

        # Validate interviewer IDs
        self._validate_interviewer_ids(data, errors)
    ...
    def _validate_candidate(self, data, request, errors):
        """Validate candidate existence and status"""
        candidate = (
            Candidate.objects.select_for_update()
            .select_related("designation__job_role")
            .filter(
    ...
    def _validate_interviewer_ids(self, data, errors):
        """Validate interviewer IDs"""
...
class CandidateFeedbackSerializer(serializers.ModelSerializer):
    ...
    round_name = serializers.CharField(source="next_round.name", read_only=True)
    specialization = serializers.CharField(source="specialization.name", read_only=True)

    def get_role(self, obj):
        return get_display_name(obj.designation.job_role.name, constants.ROLE_CHOICES)
...
class InterviewFeedbackSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        with transaction.atomic():
            ...
            if instance.overall_remark == "NJ":
                # client credit deduction for not joined canddiate
                description = f"{candidate.name}'s No Show"
                ...
                refund_points = (
                    points
                    - settings.CLIENT_LATE_RESCHEDULE_CANCEL_AND_NOT_JOINED_CREDIT_POINTS
                )
                CreditDeductionService.refund_credits(
                    client,
                    refund_points,
                    client_profile.code,
                    description,
                    reference=f"interview: {interview.id}",
                )
