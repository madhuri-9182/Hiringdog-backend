...
from hiringdogbackend.utils import (
    ...
    is_valid_pan,
    get_boolean,
    check_for_email_uniqueness,
)
from common import constants
from ..tasks import send_mail, send_email_to_multiple_recipients, get_display_name
...
class ClientPointOfContactSerializer(serializers.ModelSerializer):
    ...
    def run_validation(self, data=...):
        ...
        errors = check_for_email_uniqueness(email, User)
        if errors:
    ...
    def validate(self, data):
        ...
        is_client_admin = self.context.get("is_client_admin", False)
        if is_client_admin and not data.get("phone"):
            errors.setdefault("phone", []).append("phone is reuqired for admin")
...
class InternalClientSerializer(serializers.ModelSerializer):
    ...
    code = serializers.ChoiceField(
        choices=list(constants.COUNTRY_DETAILS.keys()),
        error_messages={
            "invalid_choice": f"This is an invalid choice. Valid choices are {', '.join(constants.COUNTRY_DETAILS.keys())}."
        },
    )
    ...
    class Meta:
        ...
        fields = (
            ...
            "address",
            "client_level",
            "initial_free_interviews_allocation",
            "points_of_contact",
            "onboarded_at",
        )
    ...
    def to_internal_value(self, data):
        ...
        for i, contact in enumerate(points_of_contact):
            context = {}
            if i == 0:
                context = {"is_client_admin": True}

            if contact.get("poc_id"):
                contact["email"] = ""
                contact_ids.append(contact.get("poc_id"))
            serializer = ClientPointOfContactSerializer(data=contact, context=context)
    ...
    def validate(self, data):
        ...
        code = data.get("code")
        if not constants.COUNTRY_DETAILS.get(code):
            errors.setdefault("code", []).append("Invalid code")
        ...
        with transaction.atomic():
            ...
            for index, point_of_contact in enumerate(points_of_contact_data):
                ...
                if index == 0:
                    ...
                    """ Below agreement is not going to use anymore, still keeping for future use"""
                    rate_values = {
                        "IN": [2500, 3000, 3200, 3600, 4500],
                        "US": [100, 120, 128, 144, 180],
                    }
                    agreement_rates = [
                        {
                            "years_of_experience": years,
                            "rate": rate_values[code][i],
                            "credits": Agreement.calculate_credits(
                                code, rate_values[code][i]
                            ),
                        }
                        for i, years in enumerate(["0-4", "4-6", "6-8", "8-10", "10+"])
                    ]
                    ...
                    """Client wallet creation"""
                    try:
                        plan = CreditPackagePricing.objects.get(
                            package__name="free", country=code
                        )
                    except CreditPackagePricing.DoesNotExist:
                        plan = None
                        log_action(
                            f"No pricing plan found for country code: {code} for client: {organization.name}",
                            logging.WARNING,
                        )

                    ClientCreditWallet.objects.get_or_create(
                        client=organization,
                        pricing_plan=plan,
                        defaults={"total_credits": 300},
                    )
    ...
    def update(self, instance, validated_data):
        ...
        for index, point_of_contact in enumerate(point_of_contact_data):
            ...
            # log_action(
            #     f"Index {index}, contact_id {contact_id}, contact_ids {contact_ids}, point_of_contact {point_of_contact}, existing_contacts_dict {existing_contacts_dict}",
            #     level=logging.DEBUG,
            # )
...
class DesignationDomainSerializer(serializers.ModelSerializer):
    ...
    def get_full_name(self, obj):
        role_choice = dict(constants.ROLE_CHOICES)
...
class StreamSerializer(serializers.ModelSerializer):
    ...
    def get_display_name(self, obj):
        return get_display_name(obj.name, constants.STRENGTH_CHOICES)


class InterviewerSerializer(serializers.ModelSerializer):
    specialization = StreamSerializer(source="stream", many=True, read_only=True)
    specialization_ids = serializers.CharField(
        max_length=100,
        required=False,
        write_only=True,
    )
    ...
    def validate(self, data):
        ...
        errors = validate_incoming_data(
            ...
            allowed_keys=["account_number", "ifsc_code", "social_links", "cv"],
            partial=self.partial,
            original_data=data,
            form=True,
        )
        ...
        if "specialization_ids" in data and isinstance(data["specialization_ids"], str):
            try:
                data["specialization_ids"] = list(
                    map(int, data["specialization_ids"].split(","))
                )
            except ValueError:
                raise serializers.ValidationError(
                    {
                        "specialization_ids": [
                            "specialization IDs must be comma-separated and consist of valid IDs."
                        ]
                    }
                )
        specialization_ids = set(data.get("specialization_ids", []))
        existing_specialization_ids = set(Stream.objects.values_list("id", flat=True))
        invalid_specialization_ids = specialization_ids - existing_specialization_ids
        if invalid_specialization_ids:
            errors.setdefault("specialization_ids", []).append(
                f"Invalid specialization IDs: {', '.join(map(str, invalid_specialization_ids))}"
            )
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            interviewer_obj.stream.add(*specialization_ids)
            verification_data = (
    ...
    def update(self, instance, validated_data):
        ...
        specialization_ids = set(validated_data.get("specialization_ids", []))
        ...
        with transaction.atomic():
            ...
            instance.stream.set(specialization_ids)
